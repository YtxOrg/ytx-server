...
pub async fn renew_vault_token(vault_addr: &str, token: &str) -> Result<(), anyhow::Error> {
    ...
    if resp.status().is_success() {
        println!("Vault token renewed successfully.");
        Ok(())
    } else {
        Err(anyhow::anyhow!(
            "Failed to renew Vault token: HTTP {}",
            resp.status()
        ))
    }
}
...
pub async fn periodic_renewal(vault_addr: String, token: String) -> Result<()> {
    ...
    loop {
        ...
        if let Err(e) = renew_vault_token(&vault_addr, &token).await {
            eprintln!("Vault token renewal failed: {:?}", e);
        } else {
            println!("Vault token renewed successfully at {:?}", Instant::now());
        }
    }
}
